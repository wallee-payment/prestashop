...
class WalleeModelTransactioninfo extends ObjectModel
{
    ...
    {
        $decoded = WalleeTools::base64Decode($this->labels, true);
        if ($decoded === false) {
            $decoded = $this->labels;
        }
    ...
    {
        $this->labels = WalleeTools::base64Encode(serialize($labels));
    }
    ...
    {
        $transactionInfos = new PrestaShopCollection('WalleeModelTransactioninfo');
        $transactionInfos->where('order_id', '=', $orderId);
        $result = $transactionInfos->getFirst();
        if ($result === false) {
            $result = new WalleeModelTransactioninfo();
        }
        return $result;
    }
